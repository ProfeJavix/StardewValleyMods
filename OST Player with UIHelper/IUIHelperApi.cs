...
namespace UIHelper
{
    public interface IUIHelperApi
    {
        ...
        ClickableTextureComponent or its son, the UI will use its following methods as you implemented them:
            -tryHover(int x, int y, float maxScaleIncrease = 0.1f)
        ...
        same as the one on your bounds, it will adjusted depending on the current element spot (your X position will be clamped
        within the limits).
        */
